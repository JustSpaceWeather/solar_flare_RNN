    ...
    for time_steps in config.time_steps_list:
        ...
        for i in range(10):
            ...
            for j in range(config.epoch):
                ...
                each_model_save_path = p + '/weights/Bi_GRU_attention_best≥C/time_steps=' + str(time_steps)
                best_TSS = show_score_and_save_weights(  # 计算最好的TSS，并保存取得最好的TSS的权重
                    model=model,
                    best_TSS=best_TSS,
                    y_true=y_true, y_pred=y_pred,
                    filename=each_model_save_path + '/Bi_GRU_attention_C_' + str(time_steps) + '_best_' + str(i) + '.h5'
                )
                print('======================================')
