...
if __name__ == '__main__':
    for i in range(10):
        (x_train, y_train, train_weight_dir), (x_test, y_test, test_weight_dir) = load_data_M(train_list[i],
                                                                                              test_list[i])
        model = get_NN_model(learning_rate=learning_rate, dropout_rate=dropout_rate,
                             glorot_normal_seed=glorot_normal_seed, score_metrics=score_metrics)
        # 评价指标初始化
        best_TSS = float('-inf')
        for j in range(epoch):
            ...
            each_model_save_path = p + '/weights/NN_best≥M_time_steps=' + str(time_steps)
            best_TSS = show_score_and_save_weights(  # 计算最好的TSS，并保存取得最好的TSS的权重
                model=model,
                best_TSS=best_TSS,
                y_true=y_true, y_pred=y_pred,
                filename=each_model_save_path + '/NN_M_best_' + str(i) + '.h5'
            )
            print('======================================')
