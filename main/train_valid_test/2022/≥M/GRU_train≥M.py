...
if __name__ == '__main__':
    for time_steps in [60, 120]:  # seed_config.time_steps_list: 30, 40, 60, 120
        is_new = True
        best_TSS_list = []  # 保存每个训练集的最好的TSS
        model_save_path = p + '/weights/2022/GRU_best≥M/time_steps=' + str(time_steps)
        for i in range(10):
            ...
            model = get_GRU_model(time_steps=time_steps, learning_rate=config.learning_rate,
            ...
            for j in range(config.epoch):
                ...
                best_TSS = show_score_and_save_weights(  # 计算最好的TSS，并保存取得最好的TSS的权重
                    ...
                    filename=model_save_path + '/GRU_M_' + str(time_steps) + '_best_' + str(i) + '.h5'
                )
