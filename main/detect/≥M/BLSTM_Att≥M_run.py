...
if __name__ == '__main__':
    for time_steps in time_steps_list:
        ...
        for i in range(10):
            ...
            # 载入模型
            model = get_Bi_LSTM_attention_model(time_steps=time_steps, learning_rate=1e-4, dropout_rate=0.0, seed=369,
                                                score_metrics=score_metrics)
            model.load_weights(
                p + r'\weights\TVT\Bi_LSTM_attention_best≥M\time_steps=' + str(time_steps) + r'\Bi_LSTM_attention_M_' + str(
                    time_steps
                ) + '_best_' + str(i) + '.h5'
            )
            # 根据时间步修改测试集shape
