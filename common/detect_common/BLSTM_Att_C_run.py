...
def BLSTM_Att_C(p, file_config, detect_type, class_type: str) -> None:
    """
    :param p: 项目根目录地址
    ...
    for time_steps in detect_config.time_steps_list:
        ...
        for i in range(10):
            ...
            model.load_weights(
                p + '/weights/' + detect_type + '/Bi_LSTM_attention_best≥' + class_type + '/time_steps=' + str(
                    time_steps) + '/Bi_LSTM_attention_' + class_type + '_' + str(time_steps) + '_best_' + str(i) + '.h5'
            )
            # 根据时间步修改测试集shape
