...
def train(p: str, file_config, data_type: str, model_name, class_type, get_model, feature_cut_list) -> None:
    """
    :param feature_cut_list: 要去掉的特征
    :param p: 根目录地址
    ...
    for time_steps in time_steps_list:
        ...
        if model_name == 'NN':
            model_save_path = p + '/weights/feature_impotence_cut/' + data_type + '/NN_best≥' + class_type + \
                              '_time_steps=' + str(time_steps) + '/' + str(len(feature_cut_list))
        else:
            model_save_path = p + '/weights/feature_impotence_cut/' + data_type + '/' + model_name + '_best≥' + \
                              class_type + '/time_steps=' + str(time_steps) + '/' + str(len(feature_cut_list))
        ...
        for i in range(len(train_list)):
            ...
            # 导入训练数据
            if class_type == 'C':
                (x_train, y_train, train_weight_dir), (x_valid, y_valid, valid_weight_dir) = load_data_cut_feature_C(
                    train_list[i], valid_list[i], feature_cut_list)
            else:
                (x_train, y_train, train_weight_dir), (x_valid, y_valid, valid_weight_dir) = load_data_cut_feature_M(
                    train_list[i], valid_list[i], feature_cut_list)
            # 根据时间步修改训练和测试数据形状
            if model_name != 'NN':
                x_train = x_train.reshape(-1, time_steps, feature_size)
                x_valid = x_valid.reshape(-1, time_steps, feature_size)
