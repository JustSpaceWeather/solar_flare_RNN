...
def LSTM_attention_C_train(p: str, file_config, train_type: str):
    """
    :param p: 根目录地址
    ...
    for time_steps in train_config.time_steps_list:
        ...
        model_save_path = p + '/weights/' + train_type + '/LSTM_attention_best≥C/time_steps=' + str(time_steps)
        for i in range(len(train_list)):
            ...
            for j in range(train_config.epoch):
                ...
                print('=================LSTM_attention_train_C.py=====================')
            save_loss(
                loss_list, val_loss_list, train_config.epoch,
                model_save_path + '/LSTM_attention_C_' + str(time_steps) + '_best_' + str(i) + '.jpg',
                is_new
            )
            is_new = False
