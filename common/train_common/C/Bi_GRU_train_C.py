...
def Bi_GRU_C_train(p: str, file_config, train_type: str):
    """
    :param p: 根目录地址
    ...
    for time_steps in train_config.time_steps_list:
        ...
        model_save_path = p + '/weights/' + train_type + '/Bi_GRU_best≥C/time_steps=' + str(time_steps)
        for i in range(len(train_list)):
            ...
            model = get_Bi_GRU_model(time_steps=time_steps, learning_rate=train_config.learning_rate,
            ...
            for j in range(train_config.epoch):
                ...
                print(train_list[i] + '\n' + valid_list[i] + '\nEpoch ' + str(j + 1) + '/' + str(
                    train_config.epoch))  # 打印当前训练的训练集和代数
                # 开始训练
                history = model.fit(
                    x_train, y_train,
                ...
                best_TSS = show_score_and_save_weights(  # 计算最好的TSS，并保存取得最好的TSS的权重
                    ...
                    filename=model_save_path + '/Bi_GRU_C_' + str(time_steps) + '_best_' + str(i) + '.h5'
                )
                loss_list.append(history.history['loss'])
                val_loss_list.append(history.history['val_loss'])
                print('=================Bi_GRU_train_C.py=====================')
            save_loss(
                loss_list, val_loss_list, train_config.epoch,
                model_save_path + '/Bi_GRU_C_' + str(time_steps) + '_best_' + str(i) + '.jpg',
                is_new
            )
            is_new = False
