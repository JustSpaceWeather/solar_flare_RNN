...
def GRU_attention_M_train(p: str, file_config, train_type: str):
    ...
    for time_steps in train_config.time_steps_list:
        ...
        model_save_path = p + '/weights/' + train_type + '/GRU_attention_best≥M/time_steps=' + str(time_steps)
        for i in range(10):
            ...
            for j in range(train_config.epoch):
                ...
                best_TSS = show_score_and_save_weights(  # 计算最好的TSS，并保存取得最好的TSS的权重
                    ...
                    filename=model_save_path + '/GRU_attention_M_' + str(time_steps) + '_best_' + str(i) + '.h5'
                )
                loss_list.append(history.history['loss'])
                val_loss_list.append(history.history['val_loss'])
                print('==================GRU_attention_train_M.py====================')
            save_loss(
                loss_list, val_loss_list, train_config.epoch,
                model_save_path + '/GRU_attention_M_' + str(time_steps) + '_best_' + str(i) + '.jpg',
                is_new
            )
            is_new = False
